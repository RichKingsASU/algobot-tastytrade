...
serve(async (req) => {
  try {
    ...
    const pathVariables: Record<string, string> = {};
        const match = requestUrl.pathname.match(new RegExp('^/instruments/future\-option\-products/([^/]+)/([^/]+)/?$'));
    if (match) {
      pathVariables.exchange = match[1];
      pathVariables.root_symbol = match[2];
    }
    ...
    let tastytradeApiUrl = `${TASTYTRADE_BASE_URL}/instruments/future-option-products/:exchange/:root_symbol`;
