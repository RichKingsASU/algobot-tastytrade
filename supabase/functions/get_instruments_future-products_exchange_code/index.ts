...
serve(async (req) => {
  try {
    ...
    const pathVariables: Record<string, string> = {};
        const match = requestUrl.pathname.match(new RegExp('^/instruments/future\-products/([^/]+)/([^/]+)/?$'));
    if (match) {
      pathVariables.exchange = match[1];
      pathVariables.code = match[2];
    }
    ...
    let tastytradeApiUrl = `${TASTYTRADE_BASE_URL}/instruments/future-products/:exchange/:code`;
