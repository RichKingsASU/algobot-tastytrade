...
serve(async (req) => {
  try {
    ...
    const pathVariables: Record<string, string> = {};
        const match = requestUrl.pathname.match(new RegExp('^/accounts/([^/]+)/orders/([^/]+)/dry\-run/?$'));
    if (match) {
      pathVariables.account_number = match[1];
      pathVariables.order_id = match[2];
    }
    ...
    let tastytradeApiUrl = `${TASTYTRADE_BASE_URL}/accounts/:account_number/orders/:order_id/dry-run`;
