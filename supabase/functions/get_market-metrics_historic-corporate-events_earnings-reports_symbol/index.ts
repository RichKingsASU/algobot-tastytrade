...
serve(async (req) => {
  try {
    ...
    const pathVariables: Record<string, string> = {};
        const match = requestUrl.pathname.match(new RegExp('^/market\-metrics/historic\-corporate\-events/earnings\-reports/([^/]+)/?$'));
    if (match) {
      pathVariables.symbol = match[1];
    }
    ...
    let tastytradeApiUrl = `${TASTYTRADE_BASE_URL}/market-metrics/historic-corporate-events/earnings-reports/:symbol`;
