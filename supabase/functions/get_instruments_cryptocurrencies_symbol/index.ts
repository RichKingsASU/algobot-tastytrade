...
serve(async (req) => {
  try {
    ...
    const pathVariables: Record<string, string> = {};
        const match = requestUrl.pathname.match(new RegExp('^/instruments/cryptocurrencies/([^/]+)/?$'));
    if (match) {
    ...
    let tastytradeApiUrl = `${TASTYTRADE_BASE_URL}/instruments/cryptocurrencies/:symbol`;
