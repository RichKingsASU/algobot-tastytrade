...
serve(async (req) => {
  try {
    ...
    const pathVariables: Record<string, string> = {};
        const match = requestUrl.pathname.match(new RegExp('^/futures\-option\-chains/([^/]+)/nested/?$'));
    if (match) {
    ...
    let tastytradeApiUrl = `${TASTYTRADE_BASE_URL}/futures-option-chains/:symbol/nested`;
