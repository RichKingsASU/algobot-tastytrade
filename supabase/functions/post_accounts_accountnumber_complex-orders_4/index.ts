...
serve(async (req) => {
  try {
    ...
    const pathVariables: Record<string, string> = {};
        const match = requestUrl.pathname.match(new RegExp('^/accounts/([^/]+)/complex\-orders/?$'));
    if (match) {
    ...
    let tastytradeApiUrl = `${TASTYTRADE_BASE_URL}/accounts/:account_number/complex-orders`;
