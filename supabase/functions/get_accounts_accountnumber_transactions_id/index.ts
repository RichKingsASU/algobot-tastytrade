...
serve(async (req) => {
  try {
    ...
    const pathVariables: Record<string, string> = {};
        const match = requestUrl.pathname.match(new RegExp('^/accounts/([^/]+)/transactions/([^/]+)/?$'));
    if (match) {
      pathVariables.account_number = match[1];
      pathVariables.id = match[2];
    }
    ...
    let tastytradeApiUrl = `${TASTYTRADE_BASE_URL}/accounts/:account_number/transactions/:id`;
